...
usage = """
    Usage:   continue_a_tpr_v3.py [prev-tprfile] [prev-jobdir] [this-jobdir] [topfile] [ndxfile] [mdpfile] [output tprfile] <ligonly>

    Add extra argument "ligonly" if this is a ligand-only simulation

    VERSION NOTES
    04-19-2020    v3: This version of this scripts, and expanded_v3.py, works with the multi-atom pull groups, and fixes
                      an issue with selecting the correct column of the WL weights from the md.log file
    04-13-2020    v2: This version of this script, and the helper object expanded_v2.py are designed to FIX problems with the umbrella restraints

    EXAMPLE
        $ python continue_a_tpr_v3.py runclone0/frame0.tpr runclone0/results0 nextout RUN0/topol.top  RUN0/index.ndx RUN0/prod.mdp  nextout/frame1.tpr
    or
        $ python continue_a_tpr_v3.py runclone0/frame0.tpr runclone0/results0 nextout RUN0/topol.top  RUN0/index.ndx RUN0/prod.mdp  nextout/frame1.tpr ligonly
...
cmd = "{GMX_BIN} grompp -c {prev_gen_tpr} -t {prev_gen_trr} -f {this_mdpfile} -p {topfile} -n {ndxfile} -o {this_gen_tpr} -po {this_jobdir}/mdout.mdp -maxwarn 40".format(GMX_BIN=GMX_BIN, prev_gen_tpr=prev_gen_tpr, prev_gen_trr=prev_gen_trr, \
