...
L_gen_cutoff = 5
wl_increment_cutoff = 1.0 # only plot ligands with avg_RL_WL_increment < N
...
for run in tqdm.tqdm(range(runs)):
    ...
    for clone_L in set(run_L['clone'].values):
        try:
            ...
            ns_L.append(int(gen_L['gen'].values[0])*10)
    ...
    for clone_RL in set(run_RL['clone'].values):
        try:
            ...
            energies_RL.append([float(x) for x in raw_energies]) # - raw_energies[0] for x in raw_energies])
...
#        plt.plot(range(len(energies)), energies)
        ...
        for lindex, lrow in clones_L.iterrows():
            for rindex, rrow in clones_RL.iterrows():
                ...
                plt.plot(range(len(clone_energy)), clone_energy)
