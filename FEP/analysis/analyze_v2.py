...
try:
    ...
    whole_dataset = pd.read_pickle('master_FEP.pkl') # master dataframe for dataset (ask matt if missing)
...
for run in tqdm.tqdm(results['run'].values):
    try:
        ...
        good_results.append([desc[0],f"{server_path}/PROJ{run_info[f'{version}_project'].values[0]}/RUN{run_info[f'{version}_run'].values[0]}",run_info['identity'].values[0],run_info['receptor'].values[0],run_info['score'].values[0], result['free_energy'].values[0], energy_errors[-1], clones_RL['length'].values, clones_L['length'].values, clones_RL['wl_increment'].values])
...
print(good_results)
good_results.to_pickle(f'results/{description}_{version}_{ts}.pkl')
